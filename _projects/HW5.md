...
The actuator applies a force \(F_{act}\) at joint **C**, creating a moment:

\[
M_{act} = F_{act} \, r_{AC} \sin(\theta_{AC})
\]

The load at point **K** produces an opposing moment:

\[
M_{load} = W \, r_{AK} \sin(\theta_{AK})
\]

Static equilibrium requires:

\[
F_{act} \, r_{AC} \sin(\theta_{AC}) = W \, r_{AK} \sin(\theta_{AK})
\]
...
- \(r_{AC} = 200\text{ mm}\)  
- \(r_{AK} = 450\text{ mm}\)  
- \(\theta_{AC}\), \(\theta_{AK}\) depend on the actuator and bar orientation.

Solving for the liftable weight:

\[
W = F_{act}\,\frac{r_{AC}}{r_{AK}} \, 
\frac{\sin(\theta_{AC})}{\sin(\theta_{AK})}
\]
